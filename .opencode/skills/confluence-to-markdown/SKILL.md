...
> Install the Atlassian MCP server for OpenCode to enable direct Confluence access.
...
> Run `atlassian_authenticate` to start the OAuth flow, then complete it in your browser.
...
- **User wants to authenticate** → invoke `atlassian_authenticate`, wait for the user to complete the OAuth flow, then retry **2.2 Fetch via MCP**
...
Return to the caller (`skill({ name: "teach" })` or `skill({ name: "sync" })`):
...
| Problem | Solution |
|---|---|
| MCP not authenticated | Run `atlassian_authenticate` to start OAuth flow |
